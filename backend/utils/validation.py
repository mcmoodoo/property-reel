...
class PropertyData(BaseModel):
    ...
    bedrooms: Optional[int] = Field(default=None, ge=0, le=20, description="Number of bedrooms")
    bathrooms: Optional[float] = Field(default=None, ge=0, le=20, description="Number of bathrooms")
    square_feet: Optional[int] = Field(default=None, ge=100, le=50000, description="Square footage")
    ...
    description: Optional[str] = Field(default=None, max_length=2000, description="Property description")
    
    @validator('property_type')
    def validate_property_type(cls, v):
        valid_types = ['residential', 'commercial', 'condo', 'townhouse', 'apartment', 'land']
...
def validate_video_files(files: List[UploadFile]) -> List[str]:
    ...
    for i, file in enumerate(files):
        ...
        valid_extensions = ['.mp4', '.mov', '.avi', '.mkv', '.MP4', '.MOV', '.AVI', '.MKV']
        if not any(file.filename.lower().endswith(ext.lower()) for ext in valid_extensions):
            errors.append(f"File {i+1} ({file.filename}) has unsupported format. Allowed: {valid_extensions}")
