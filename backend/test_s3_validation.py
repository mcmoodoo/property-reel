...
def create_mock_file(filename: str, content_type: str = "video/mp4", size: int = None) -> Mock:
...
async def test_file_validation():
    ...
    for filename, content_type in valid_files:
        mock_file = create_mock_file(filename, content_type, size=50*1024*1024)  # 50MB
...
async def test_error_handling():
    ...
    try:
        oversized_file = create_mock_file("huge.mp4", "video/mp4", size=1024*1024*1024)  # 1GB
