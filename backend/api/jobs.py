...
from utils.validation import PropertyData, JobResponse, JobStatus, validate_video_files, validate_job_id
...
async def create_processing_job(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid property data: {str(e)}")
        ...
        metrics = JobMetrics(
            ...
            total_video_size_mb=sum(file.size for file in files if hasattr(file, 'size')) / 1024 / 1024
        )
        db.add(metrics)
        db.commit()
        ...
        try:
            if 'clips' in job.result_s3_urls:
                result_url = await s3_service.generate_presigned_url(
                    job.result_s3_urls['clips'][0] if isinstance(job.result_s3_urls['clips'], list) 
