...
async def runpod_webhook(
    ...
    try:
        ...
        logger.info(
            f"Received RunPod webhook for job {job_id}: {payload.get('status', 'unknown')}"
        )

        # Process the webhook in the background
        background_tasks.add_task(
            process_runpod_webhook, job_id=job_id, payload=payload, db_session=db
        )
...
async def process_runpod_webhook(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Failed to update job status after webhook error: {commit_error}"
            )
...
async def handle_job_completion(
    ...
    try:
        ...
        if "clips" in output:
            ...
            clips_count = (
                len(output["clips"]) if isinstance(output["clips"], list) else 1
            )
